 ...
 * @author Evening
 */
public class ModifyCusFrame extends JFrame implements ActionListener {
    ...
    private CusManFrame parentFrame; // 父窗口引用
    private int customerId; // 要修改的客户ID

    /**
     * 构造函数
     * @param parentFrame 父窗口 (CusManFrame) 的引用
     * @param customerId 要修改的客户的ID
     */
    public ModifyCusFrame(CusManFrame parentFrame, int customerId) {
        ...
        panel.add(modifyButton); panel.add(cancelButton);
    ...
    /**
     * 根据customerId从数据库加载客户数据并填充到表单中
     */
    private void loadCustomerData() {
        Customer c = customerDAO.getCustomer(this.customerId);
        if (c != null) {
            nameText.setText(c.getCustomName());
            passText.setText(c.getCustomPassword());
            ageText.setText(String.valueOf(c.getAge()));
            addressText.setText(c.getAddress());
            phoneText.setText(c.getPhone());
            emailText.setText(c.getEmail());
        } else {
            JOptionPane.showMessageDialog(this, "无法加载客户数据，该客户可能已被删除。", "错误", JOptionPane.ERROR_MESSAGE);
            this.dispose(); // 加载失败则关闭窗口
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == modifyButton) {
            ...
            // 4. 调用DAO进行数据库更新操作
            boolean success = customerDAO.modifyCustomerInfo(this.customerId, customer);

            // 5. 根据结果给出反馈
            if (success) {
                JOptionPane.showMessageDialog(this, "客户信息修改成功！");
                parentFrame.refreshTableData(); // 回调父窗口的刷新方法
                this.dispose(); // 关闭当前窗口
            } else {
                JOptionPane.showMessageDialog(this, "修改失败，可能发生了数据库错误。", "操作失败", JOptionPane.ERROR_MESSAGE);
            }
